...
appId="258402db0c10ba8cb3e7ad13a2204852"
secretKey="47a0337a632177ce686619461542259f9a16796d"
...
def bookseats(request,starting=""):
    global name, pass_booked_bus_name, pass_booked_fees, offers,amount,ticket_key,index,depature,arrive,date,ticket,number,count,counter
    print(count)
    counter.clear()
    if(starting=="passenger"):
        ...
        for i in range(int(number)):
            counter.append(i+1)
        return render(request,'passenger.html',{'price':amount,'count':counter,'validation':count,'number':int(number)})
        ...
        if(request.method=='POST'):
            ...
            for i in range(4):
                ...
                ticket += random.choice(ticket_key)
            if(membership_option=='no'):
                ...
                data={
                    ...
                    'to':arrive,
                    'date':date,
                    'membership':membership_option
                }
               # settings.fire.collection('Tickets').add(data)

                return render(request, 'passenger.html',
                              context={'url': response, 'price': amount,'count':counter,'validation':count,'number':int(number)})
